...
// ΓöÇΓöÇ SUB-COMPONENT: CASE LIST ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const CasesList = () => {
...
// ΓöÇΓöÇ SUB-COMPONENT: CASE DETAILS & OVERRIDE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
const CaseDetails = () => {
  const { id } = useParams();
  const navigate = useNavigate();
  const { user } = useAuthStore();
          ...
          <div className="glass-card p-6 md:p-8 rounded-2xl border border-zinc-800 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm border-t border-zinc-800 pt-6">
              <div>
                ...
                <span className="font-semibold text-zinc-200">{c?.rc_no || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">SID Number</span>
                <span className="font-semibold text-zinc-200">{c?.sid_no || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Theft From (Location Subtype)</span>
                <span className="font-semibold text-zinc-200">{c?.theft_from || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Time of Theft (Time Band)</span>
                <span className="font-semibold text-zinc-200">{c?.time_of_theft || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Motive</span>
                <span className="font-semibold text-zinc-200">{c?.motive || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Hotspot?</span>
                <span className="font-semibold text-zinc-200">{c?.hotspot || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Stolen Article Type</span>
                <span className="font-semibold text-zinc-200">{c?.stolen_article_type || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Weapon Used?</span>
                <span className="font-semibold text-zinc-200">{c?.weapon_used || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Vehicle Used?</span>
                <span className="font-semibold text-zinc-200">{c?.vehicle_used || 'ΓÇö'}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm border-t border-zinc-800 pt-6">
              ...
              <div>
                ...
                <span className="font-semibold text-zinc-200">{c?.agency_order_date || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Pending Investigation Age</span>
                <span className="font-semibold text-zinc-200">{c?.pending_investigation_age || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Victim Mobile</span>
                <span className="font-semibold text-zinc-200">{c?.victim_mobile || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">IO Rank</span>
                <span className="font-semibold text-zinc-200">{c?.io_position || 'ΓÇö'}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className="font-semibold text-zinc-200">{c?.accused_victim_relation || 'ΓÇö'}</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-sm border-t border-zinc-800 pt-6">
              ...
              <div>
                ...
                <span className="font-semibold text-zinc-200">{c?.date_of_work_out || 'ΓÇö'}</span>
              </div>
              <div>
                <span className="text-zinc-500 text-xs block mb-1">Disposed Date</span>
                <span className="font-semibold text-zinc-200">{c?.disposed_date || 'ΓÇö'}</span>
              </div>
              <div className="md:col-span-3">
                <span className="text-zinc-500 text-xs block mb-1">Pending Investigation Reason</span>
                <span className="font-semibold text-zinc-200">{c?.pending_investigation_reason || 'ΓÇö'}</span>
              </div>
            </div>
                  ...
                  <tbody>
                    {details.accusedList.map((acc, idx) => (
                      <tr key={idx} className="border-b border-zinc-850 hover:bg-zinc-900/20">
                        ...
                        <td className="p-3">{acc.father_name || 'ΓÇö'}</td>
                        <td className="p-3">{acc.address || 'ΓÇö'}</td>
                        <td className="p-3">{acc.arrest_date || 'ΓÇö'}</td>
                        <td className="p-3">{acc.age || 'ΓÇö'}</td>
                        <td className="p-3">{acc.state_origin || 'ΓÇö'}</td>
                        <td className="p-3">{acc.recovery_details || 'ΓÇö'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
