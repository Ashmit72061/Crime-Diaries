...
const runMigrations = async () => {
  ...
  try {
    ...
    // Guard: Knex migration 20260621000000_record_linkage.js also creates this table.
    // Using CREATE TABLE IF NOT EXISTS keeps this script idempotent regardless of run order.
    if (!(await db.schema.hasTable('link_type_registry'))) {
      await db.raw(`
      ...
      `);
      logger.info('Table link_type_registry created');
    } else {
      logger.info('Table link_type_registry already exists, skipping create');
    }
    ...
    // Guard: same as above — Knex migration also creates this table.
    if (!(await db.schema.hasTable('record_links'))) {
      await db.raw(`
      ...
      logger.info('Table record_links created');
    } else {
      logger.info('Table record_links already exists, skipping create');
    }
