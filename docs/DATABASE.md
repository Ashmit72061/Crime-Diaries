...
| minor_category | varchar(100) | Sub-type | `"Honda Activa"` / `"Samsung Galaxy"` |
| status | varchar(20) | Current state | `"Stolen"` / `"Recovered"` / `"Seized"` |
...
**Step 1 — DB config:**
A field row in `field_registry` has a `show_when` value:

```json
{ "field": "act_name", "value": "IPC" }
```

or with multiple trigger values:
...
`FormSection.jsx` checks before rendering each field:
```
if field.show_when:
    current_value = form_values[field.show_when.field]
    if current_value does NOT match field.show_when.value → hide the field
```
The check is case-insensitive. `value` can be a single string or an array of strings.
...
```
id          | record_id | major_category | minor_category | status | details
------------+-----------+----------------+----------------+--------+-----------------------------
"pr-uuid.." | "r-uuid"  | Mobile Phone   | Samsung        | Stolen | Black Samsung Galaxy S23
```

Note: phone-specific fields (`phone_make`, `phone_model`, etc.) from the properties array are stored in the `record_properties.details` text column as a JSON blob (or in the `minor_category` for searchable sub-type). Full property data is retrievable from the frontend's local state for editing.
...
1. Open `backend/seeds/01_fields.js`
2. Add a row to the `fields` array with at minimum: `field_key`, `field_type`, `label_en`, `label_hi`, `applicable_record_types`, `section`, `sort_order`
3. If it should only show conditionally, add `show_when: JSON.stringify({ field: 'trigger_key', value: 'X' })`
4. If it belongs to a repeater, add `repeater_entity: 'PROPERTY'` (or the relevant entity name)
5. Run `npm run db:seed` to push it to the database
6. The field appears in the frontend automatically — no frontend code change needed

## Adding a New Record Type — Quick Reference

1. Add fields with `applicable_record_types: ['YOUR_TYPE']` to `01_fields.js`
